...
contract PoolContrcat is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event SetToken(address indexed usdcToken);
    event AddTreasuery(uint256  indexed _treasuryPoolAmount);
    event OwnershipAdded(uint256 indexed _ownerShipPoolAmount);
    event AddFunds(uint256 indexed _amount, uint256 indexed _projectNo);
    event Withdraw (address indexed recipient, uint256 indexed usdcAmount);
    event SetPermission(bool indexed _permission, bool indexed permission);
    event OwnerChanged (address indexed previousOwner, address indexed newOwneer);
    event PercentageChanged(address indexed _owner, uint256 indexed _newPercentage);
    event SetForwarderAddress(address indexed _owner, address indexed _rewardSender);
    event multipleUserWhiteListed(address indexed owner, uint256 indexed usersLength);
    event UserBlocked(address indexed owner,address indexed blockUserAddress, bool blocked);
    event AmountDeductedByOwner(address indexed owner, address indexed user, uint256 amount);
    event StakeTokens (address indexed sender, address indexed recepient,uint256 usdcAmount);
    event AddProject(uint256 indexed projectId, uint256 indexed OpPercentage,uint256 tpPercentage);
    event multipleUserAddeed(address indexed owner,uint256 indexed _amountLength, uint256 usersLength);
    event WeeklyTransfered(address indexed caller,uint256  indexed ownerShipPoolAmount, uint256  treasuryPoolAmount, uint256 noOfUsers,
        ...
        initializer public
    {
        ...
        usdcToken = IBEP20(_usdcAddress);
            ...
            thirdOwner = _multisigAddress;
            rewardSender = _rewardSender;
            permission = true;
            whiteListed[owner()] = true;
            whiteListed[thirdOwner] = true;
            whiteListed[secondryOwner] = true;
    }


     function addProjects(uint256 _tPPercentage) external allOwners(){
    ...
    function addOwnershipPool(uint256 _amount) external {
    ...
    function addTreasueryPool(uint256 _amount) external {
    ...
    function stakeTokensByOwner(uint256[] memory _amount, address[] memory users) external allOwners() {
    ...
    function deductAmountByOwner(uint256 _amount, address user) external allOwners() {
        ...
        if (userRegistered[user].blocked) {
            revert userBlocked(userRegistered[user].blocked);
        }

        if (userRegistered[user].totalStakedAmount < _amount) {
            revert notEnoughBalance(_amount);
        }

        userRegistered[user].totalStakedAmount -= _amount;
        totalStakedAmount -= _amount;

        emit AmountDeductedByOwner(msg.sender, user, _amount);
    }
    ...
    function weeklyTransfer() public PermissionGranted()  {

        if(msg.sender != rewardSender && msg.sender != owner() && msg.sender != secondryOwner && msg.sender != thirdOwner){
    ...
    function setRewardSender(address _rewardSender) external allOwners() {
    ...
    function ownerWithdrawAmoount(uint256 _amount) external nonReentrant() allOwners() {
    ...
    function blockUser(address _userAddress, bool value) external allOwners(){
    ...
    function setTeasueryPercentages(uint256 _projectId,uint256 _newPerccentage) external allOwners() {
    ...
    function settdividentPayoutPercentage(uint256 _newPerccentage) external allOwners() {
    ...
    function OPpercentageDistribution(
        ...
        uint256 _odividentPayoutPercentage) external allOwners() {
    ...
    function changeOwnerShipTwo(address ownerAddressTwo) external allOwners(){
    ...
    function changeOwnerShipThree(address ownerAddressThree) external allOwners(){
        
        if(ownerAddressThree == address(0)){
            revert wrongAddress(ownerAddressThree);
        }
        
        address previousOwner = thirdOwner;
        thirdOwner = ownerAddressThree;
       
        emit OwnerChanged(previousOwner, thirdOwner);
    }

    function setPermission(bool _permission) external allOwners() {
    ...
    function setToken(address  _tokenAddress) external allOwners() {
    ...
    modifier allOwners(){
        if(msg.sender != owner() && msg.sender != secondryOwner && msg.sender != thirdOwner){
            revert wrongOwner(msg.sender);
        }
        _;
    }
