...
contract PoolContrcat is Initializable, OwnableUpgradeable, UUPSUpgradeable, AutomationCompatibleInterface {
    ...
    event offInterval(address _owner, uint256 _interval,bool startAgain);
    ...
    function stakeTokensByOwner(uint256[] memory _amount, address[] memory users) external bothOwner() {
        ...
        for(uint i=0 ;i < _amount.length; i++){
            ...
            userRegistered[users[i]].totalStakedAmount += (_amount[i] * 1 ether);
            totalStakedAmount += (_amount[i] * 1 ether);
        }
    ...
    function off () external  bothOwner{
        ...
        interval = 0;
        checkOnce = false;
        emit offInterval(msg.sender, interval, checkOnce);
    }
    ...
    function setmaintainceFeePercentage(uint256 _newPerccentage) external bothOwner {
     ...
     function changeOwnerShipOne(address ownerAddreessOne) external bothOwner{
        
        if(ownerAddreessOne == address(0)){
            revert wrongAddress(ownerAddreessOne);
        }

        address previousOwner = ownerOne;
        ownerOne = ownerAddreessOne;

        emit OwnerChanged(previousOwner, ownerAddreessOne);
    ...
    function changeOwnerShipTwo(address ownerAddressTwo) external bothOwner{
        ...
        address previousOwner = ownerTwo;
        ownerTwo = ownerAddressTwo;
