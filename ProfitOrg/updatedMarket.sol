...
contract Marketplace is 
    ...
    error allCopiesSold(uint256 noOfcopies);
    ...
    function BuyFixedPriceItem(uint256 _listId, uint256 _noOfCopies) payable external checkSell(_listId)  { 
        ...
        if(_noOfCopies <=  0 || _noOfCopies > 1){
            revert invalidNoOfCopies(_noOfCopies);
        }
        
        if(listing[_listId].noOfCopies < 1){
            revert allCopiesSold(listing[_listId].noOfCopies);
        }

        if(msg.value <  listing[_listId].price || msg.value <  listing[_listId].price){
