...
contract Wager is Permit, EIP712, Nonces, Ownable {
    ...
    event AllowanceForSell(address _user, address _owner,uint256 shares,uint256 sellPrice);
   ...
   constructor(
    ...
    function BetPermit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH1, _user, owner, value, betOn, _useNonce(_user), deadline));
    ...
    function SellSharePermit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH2, _user, owner, _noOfShares, _price, _useNonce(_user),_sellOf, deadline));
        bytes32 hash = _hashTypedDataV4(structHash);
    ...
    function setSellInfo(
        ...
        if(_sellOf == 0){
            ...
            if(_noOfShares > userInfo[_user].noShareAmount){
                revert notEnoughAmount(userInfo[_user].noShareAmount);
            }

            sellInfo[_user][userInfo[_user].listNo].noShare = _noOfShares; 

        }else{

            if(_noOfShares > userInfo[_user].yesShareAmount){
                revert notEnoughAmount(userInfo[_user].yesShareAmount);
            }
    ...
    function cancelSellSharePermit(
        address _user,
        address owner,
        uint256 _listNo,
        uint256 deadline,
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH3, _user, owner, _listNo, _useNonce(_user),deadline));
    ...
    function cancelSellShare(
        ...
        validAddress(_user,owner)
        checkBuyShare(_user,_listNo) 
    ...
    private {
        ...
        setCancelInfo( _user, _listNo);
    
        emit CancelSellShare(_user,owner, _listNo);
    }

    function setCancelInfo(address _user, uint256 _listNo) private {

        sellInfo[_user][_listNo].list = false;
        sellInfo[_user][_listNo].price = 0;
        sellInfo[_user][_listNo].listOn = 0;
        sellInfo[_user][_listNo].owner = address(0);
    ...
    function BuySharePermit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH4, _user, owner, _listNo,_listedOwner, _useNonce(_user),deadline));
    ...
    function buyShare(
        ...
        validAddress(_user,_owner)
        onlyAllowancer(_owner)
        checkBuyShare(_listedOwner,_listNo)
    ...
    public {
        ...
        uint256 _noOfshares = setBuyInfo(_user,_owner, _listedOwner, _listNo);
        ...
        bool success = usdcToken.transferFrom(
            _user,
            _listedOwner,
            sellInfo[_listedOwner][_listNo].price
        );
    ...
    function setBuyInfo(address _user,address _owner, address _listedOwner, uint256 _listNo) private returns (uint256 _noOfshares) {
        
        if(sellInfo[_listedOwner][_listNo].listOn == 0){
            
            _noOfshares =  sellInfo[_listedOwner][_listNo].noShare;   

            userInfo[_listedOwner].noShareAmount -= _noOfshares;
            userInfo[_user].noShareAmount += _noOfshares;
            userInfo[_user].noBetAmount += sellInfo[_listedOwner][_listNo].price;

        }else{

            _noOfshares = sellInfo[_listedOwner][_listNo].yesShare;

            userInfo[_listedOwner].noShareAmount -= _noOfshares;
            userInfo[_user].yesShareAmount += _noOfshares;
            userInfo[_user].yesBetAmount += sellInfo[_listedOwner][_listNo].price;
        }

        userInfo[_user].betOn[sellInfo[_listedOwner][_listNo].listOn] = true;
        sellInfo[_listedOwner][_listNo].sold = true;
    ...
    function draw() external isMarketReesolved()  {
    ...
    modifier validAddress(address _user, address _owner) {
    ...
    modifier checkBuyShare(address _owner,uint256 _listNo) {
        
        if(_listNo == 0){
            revert wrongListNo(_listNo);
        }

        if(!sellInfo[_owner][_listNo].list){
            revert notListed(sellInfo[_owner][_listNo].list);
        }
