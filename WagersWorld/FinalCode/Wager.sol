...
contract Market is Ownable {
    ...
    function buyShare(uint256 _listNo, address _owner) external {
        ...
        if(sellInfo[_owner][_listNo].listOn == 0){
            ...
            uint256 deductAmount = calculateAmount(sellInfo[_owner][_listNo].noShare, 0);

            userInfo[msg.sender].noBetAmount += deductAmount;
            userInfo[_owner].noBetAmount -= deductAmount;

        }else{

            userInfo[msg.sender].noShareAmount += sellInfo[_owner][_listNo].noShare;
            
            uint256 deductAmount = calculateAmount(sellInfo[_owner][_listNo].noShare, 1);
    ...
    function calculateAmount(uint256 _shares, uint256 _betOn) public view returns (uint256) {
        
        uint256 price = marketInfo[address(this)].initialPrice[_betOn];
        require(price != 0, "_price cannot be zero");

        uint256 amount = (_shares * price) / 100;
        return amount;
    }
