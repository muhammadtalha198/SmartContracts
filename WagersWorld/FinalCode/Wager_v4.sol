...
contract MyContract is Permit, EIP712, Nonces, Ownable {
    ...
    event SelllShares(address indexed user, uint256 listNo,  uint256 onPrice);
   ...
   constructor(
    ...
    function permit(
     ...
     bytes32 private constant PERMIT_TYPEHASH1 = keccak256("betPermit(address _user,address owner,uint256 value,uint256 betOn,uint256 nonce,uint256 deadline)");
    ...
    function PriceCalculation() public view returns(uint256 , uint256 ) {
     ...
     bytes32 private constant PERMIT_TYPEHASH2 = keccak256("sellSharePermit(address _user,address owner,uint256 _noOfShares,uint256 _price, uint256 _sellOf,uint256 nonce,uint256 deadline)");
    ...
    function sellSharePermit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH2, _user, owner, _noOfShares, _price, _useNonce(_user)));
    ...
    function sellShare(address _user, address _owner,uint256 _noOfShares, uint256 _price, uint256 _sellOf) public {
     ...
     bytes32 private constant PERMIT_TYPEHASH3 = keccak256("cancelSellSharePermit(address _user,address owner,uint256 _listNo,uint256 nonce,uint256 deadline)");
    ...
    function cancelSellShare(address _user, address _owner,uint256 _listNo) public {
        ...
        if (sellInfo[_user][_listNo].listOn == 0) {
            userInfo[_user].noShareAmount += sellInfo[_user][_listNo].noShare;
        } else {
            userInfo[_user].yesShareAmount += sellInfo[_user][_listNo].yesShare;
        }
     ...
     bytes32 private constant PERMIT_TYPEHASH4 = keccak256("buySharePermit(address _user,address owner,uint256 _listNo,address _listedOwner,uint256 nonce,uint256 deadline)");
    ...
    function buySharePermit(
        ...
        bytes32 structHash = keccak256(abi.encode(PERMIT_TYPEHASH3, _user, owner, _listNo,_listedOwner, _useNonce(_user),deadline));
